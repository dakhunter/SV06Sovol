...
class ExtrudePanel(ScreenPanel):
    ...
    def __init__(self, screen, title):
        ...
        if self.ks_printer_cfg is not None:
            ...
            if re.match(r'^[0-9,\s]+$', dis):
                ...
                if 1 < len(dis) < 5:
                    self.distances = dis
            vel = self.ks_printer_cfg.get("extrude_speeds", '1, 5, 10, 15')
    ...
    def change_extruder(self, widget, extruder):
        ...
        self._screen._ws.klippy.gcode_script(f"M104 T{self._printer.get_tool_number(extruder)} S200")
