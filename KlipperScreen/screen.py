...
from panels.zone import ZonePanel
from panels.update_state_warn import UpdateStateWarnPanel
...
class KlipperScreen(Gtk.Window):
    ...
    max_retries = 4
    initialized = False
    def check_update_content(self, filename):
        try:
            with open(filename, 'r') as file:
                content = file.read()
                if "updating" in content:
                    logging.debug("deb is updating")
                    return True
                else:
                    return False
        except FileNotFoundError:
            logging.exception("no updating state file found")
            return False
    ...
    def __init__(self, args, version):
        ...
        self.dialogs = []
        self.confirm = None
        update_file_path = "/home/mks/.update_state"
        ...
        deb_update_state = self.check_update_content(update_file_path)

        if deb_update_state:
            self.update_state_panel = UpdateStateWarnPanel(self, title="Firmware Updating")
            self.base_panel = BasePanel(self, title="Base Panel")
            self.add(self.update_state_panel.fixed)
            self.show_all()
        else:    
    ...
    def timezone(self):
        self.close_screensaver()
        self.close_popup_message()
        self.show_panel('zone', "zone", None, 2)
    ...
    def read_varibles_cfg_value(self, option):
        ...
        _config.read('/home/mks/printer_data/config/saved_variables.cfg')
    ...
    def modify_cfg_value(self, option, new_value):
        ...
        with open('/home/mks/printer_data/config/saved_variables.cfg', 'w') as file:
            _config.write(file)
