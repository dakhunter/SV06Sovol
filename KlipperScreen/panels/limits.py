...
from ks_includes.widgets.keypad_limit import Keypad
...
class LimitsPanel(ScreenPanel):
    ...
    def update_option(self, option, value):
        ...
        self.limits[option]['entry'].set_label(str(self.values[option]))
        for opt in self.options:
    ...
    def add_option(self, option, optname, units, value):
        ...
        entry = self._gtk.Button()
        entry.set_label(str(int(scale.get_value())))
        entry.connect("clicked", self.show_numpad, option)
        ...
        item.attach(entry, 1, 1, 1, 1)
        item.attach(reset, 2, 1, 1, 1)
    ...
    def set_opt_value(self, widget, event, opt):
        ...
        print(f'value: {value}')
        self.limits[opt]['entry'].set_label(str(float(value)))

        self.set_velocity_limit(opt, value)
    def hide_numpad(self, widget=None):
        for _ in range(2):
            self.grid.remove_row(0)
        i = 0
        for opt in self.options:
            self.grid.attach(self.limits[opt['option']]['row'], 0, i, 1, 1)
            i += 1
        self.grid.show_all()
    def show_numpad(self, widget, opt, device=None,):
        for _ in range(4):
            self.grid.remove_row(0)
        self.key_name = opt
        input_limits = 0
        if opt == "max_accel":
            ...
            input_limits = 2
        if "keypad" not in self.limits[opt] and input_limits != 0:
            self.limits["keypad"] = Keypad(self._screen, self.change_velocity_limit_value, self.hide_numpad, input_limits)
        self.limits["keypad"].clear()
        self.grid.attach(self.limits[opt]['row'], 0, 0, 1, 1)
        self.grid.attach(self.limits["keypad"], 0, 1, 1, 1)
        self.grid.show_all()
    def set_velocity_limit(self, opt, value):
        if opt == "max_accel":
    ...
    def change_velocity_limit_value(self, acc):
        print(f"acc: {self.key_name, acc}")
        self.limits[self.key_name]['entry'].set_label(str(acc))
        self.limits[self.key_name]['scale'].set_value(acc)
        self.set_velocity_limit(self.key_name, acc)