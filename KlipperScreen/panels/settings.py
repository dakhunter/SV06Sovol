...
class SettingsPanel(ScreenPanel):
    ...
    def add_option(self, boxname, opt_array, opt_name, option):
        ...
        if option['type'] == "binary":
            ...
            continent = self._config.get_config()['main'].get('timezone', 'UTC')
            city = self._config.get_config()['main'].get('timezone_city', 'UTC')
            _label = _(continent) + " - " + _(city)
            button = self._gtk.Button(label=_label)
            button.get_style_context().add_class("button_timezone")
            button.set_hexpand(False)
            button.set_halign(Gtk.Align.END)
            button.connect("clicked", self.timezone, None, None)
            dev.add(button)
    ...
    def timezone(self, widget, name, title):
